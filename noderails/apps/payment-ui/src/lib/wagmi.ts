...
import { getLeanRpcUrl, getSolanaPublicRpcUrl, NODE_RAILS_SOLANA_CHAIN_IDS, NODE_RAILS_SUI_CHAIN_IDS } from '@noderails/common';
...
/** Wagmi only supports EVM; Solana/Sui NodeRails chain IDs must never be passed as viem chains. */
...
/** Map NodeRails Solana chain id → Satellite / Wallet Standard cluster moniker. */
...
/** RPC URLs for Satellite Solana adapter — session overrides win per cluster. */
export function buildSatelliteSolanaRpcUrls(
  ...
  for (const chain of chainInputs) {
    ...
    if (!url) continue;
    const cluster = solanaClusterForChainId(chain.chainId);
    rpcUrls[cluster] = url;
  }
